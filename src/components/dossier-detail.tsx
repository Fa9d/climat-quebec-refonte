...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h2 className="mb-6 font-display text-3xl text-foreground">{children}</h2>;
}
...
function Tag({ children, color = "accent" }: { children: React.ReactNode; color?: "accent" | "eau" | "danger" }) {
  ...
  return <span className={`inline-block rounded-full px-3 py-1 text-xs font-bold ${styles[color]}`}>{children}</span>;
}
...
function BlocProbleme({ dossier }: { dossier: DossierComplet }) {
  return (
    <div className="rounded-xl border border-border bg-surface p-8">
      <h3 className="mb-1 text-xs font-bold uppercase tracking-widest text-eau dark:text-[#8fd0e6]">Le problème complet</h3>
      <p className="mt-3 text-base leading-relaxed text-muted">{dossier.contexteComplet}</p>
...
function BlocProposition({ dossier }: { dossier: DossierComplet }) {
  return (
    <div className="rounded-xl border border-accent/30 bg-accent/5 p-8">
      <h3 className="mb-1 text-xs font-bold uppercase tracking-widest text-accent">Notre proposition</h3>
      <p className="mt-3 text-base leading-relaxed text-foreground">{dossier.proposition}</p>
    </div>
  );
}
...
function BlocArguments({ dossier }: { dossier: DossierComplet }) {
  return (
    <div className="rounded-xl border border-border bg-surface p-8">
      <h3 className="mb-1 text-xs font-bold uppercase tracking-widest text-primary">Nos arguments</h3>
    ...
    <div className="rounded-lg border border-border bg-surface overflow-hidden">
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <span className="text-sm font-semibold text-foreground leading-relaxed">&laquo;&nbsp;{ca.argument}&nbsp;&raquo;</span>
        </div>
        <span className={`shrink-0 text-muted transition-transform duration-200 ${open ? "rotate-180" : ""}`} aria-hidden="true">▾</span>
      </button>
      {open && (
        <div className="border-t border-border bg-primary/5 px-6 pb-6 pt-4">
          <p className="text-xs font-bold uppercase tracking-widest text-primary mb-3">Notre réponse</p>
...
function BlocContrArguments({ dossier }: { dossier: DossierComplet }) {
  return (
    <div>
      <p className="mb-5 text-sm text-muted">Cliquez sur une attaque pour voir l&apos;argumentaire préparé.</p>
      <div className="space-y-3">
        {dossier.contrArguments.map((ca, i) => <ContrArgumentCard key={i} ca={ca} index={i} />)}
      </div>
    </div>
  );
}
    ...
    <div className="rounded-lg border border-border bg-surface overflow-hidden">
      ...
      {open && (
        <div className="border-t border-border space-y-4 bg-surface-2 px-6 py-5 text-sm">
          <p><span className="font-bold text-muted uppercase tracking-widest text-xs">Organisation&nbsp;</span><span className="text-foreground">{acteur.organisation}</span></p>
          {acteur.liensFinanciers && <p><span className="font-bold text-muted uppercase tracking-widest text-xs">Liens financiers&nbsp;</span><span className="text-foreground">{acteur.liensFinanciers}</span></p>}
          {acteur.liensPartisans && <p><span className="font-bold text-muted uppercase tracking-widest text-xs">Liens partisans&nbsp;</span><span className="text-foreground">{acteur.liensPartisans}</span></p>}
          {acteur.porteTournante && <p><span className="font-bold text-eau uppercase tracking-widest text-xs">Porte tournante&nbsp;</span><span className="text-foreground">{acteur.porteTournante}</span></p>}
...
function BlocActeurs({ dossier }: { dossier: DossierComplet }) {
  return (
    <div className="space-y-3">
      {dossier.acteursAdverses.map((a, i) => <ActeurCard key={i} acteur={a} />)}
    </div>
  );
}
        ...
        <ul className="space-y-2">
          {e.registresAConsulter.map((r, i) => (
            <li key={i} className="flex items-start gap-2 text-xs text-foreground leading-relaxed">
              <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-accent" aria-hidden="true" />{r}
            </li>
          ))}
        </ul>
      </div>
      <div className="rounded-xl border border-border bg-surface p-6">
        ...
        <ul className="space-y-2">
          {e.questionsAClarifier.map((q, i) => (
            <li key={i} className="flex items-start gap-2 text-xs text-foreground leading-relaxed">
              <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-eau" aria-hidden="true" />{q}
            </li>
          ))}
        </ul>
      </div>
      <div className="rounded-xl border border-border bg-surface p-6">
        ...
        <ul className="space-y-2">
          {e.documentsPublicsClefs.map((d, i) => (
            <li key={i} className="flex items-start gap-2 text-xs text-foreground leading-relaxed">
              <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" aria-hidden="true" />{d}
            </li>
          ))}
        </ul>
      </div>
    ...
    <div className="mx-auto max-w-5xl px-5 py-12 sm:px-8">
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-6">
        <div>
          ...
          <h1 className="font-display text-5xl text-foreground sm:text-6xl">{dossier.titre}</h1>
        </div>

        {/* CTA témoignage */}
        <Link
          href={`/temoigner?dossier=${dossier.slug}`}
          className="inline-flex items-center gap-2 rounded-full border border-accent px-5 py-2.5 text-sm font-bold text-accent transition-colors hover:bg-accent hover:text-accent-fg"
        >
      ...
      <div className="min-h-64">
        ...
        {onglet === "acteurs" && (
          <div>
            ...
            <p className="mb-6 text-sm text-muted">Cartographie des acteurs adverses : liens financiers, partisans et portes tournantes.</p>
        ...
        {onglet === "enquete" && (
          <div>
            ...
            <p className="mb-6 text-sm text-muted">Registres publics, questions à clarifier et documents à obtenir pour démasquer jusqu&apos;à la racine.</p>
      ...
      <div className="mt-12 flex flex-wrap items-center gap-4 border-t border-border pt-8">
        ...
        <Link
          href="/agir"
          className="inline-flex items-center gap-2 rounded-full border border-border px-6 py-3 text-sm font-bold text-foreground transition-colors hover:bg-surface-2"
        >
          S&apos;impliquer
          <span aria-hidden="true">→</span>
        </Link>
